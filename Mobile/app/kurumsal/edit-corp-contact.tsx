...
export default function EditCorpContactScreen() {
  ...
  const handleSave = async () => {
    if (!phoneNumber.trim()) {
      Alert.alert('Uyarı', 'Lütfen yeni kurumsal telefon numarasını giriniz.');
      return;
    }
    ...
    try {
      ...
      if (userId && email) {
        ...
        Alert.alert('Başarılı', `Kurumsal iletişim numaranız (${finalPhone}) başarıyla güncellendi.`);
      }
    } catch (error) {
      console.error('Güncelleme hatası:', error);
      Alert.alert('Hata', 'İletişim bilgileri güncellenirken bir sorun oluştu.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea, currentTheme === 'dark' && { backgroundColor: '#0f172a' }]}>
      ...
      <View style={[styles.header, currentTheme === 'dark' && { backgroundColor: '#1e293b', borderBottomColor: '#334155' }]}>
        ...
        <Text style={[styles.headerTitle, currentTheme === 'dark' && { color: '#fff' }]}>Kurumsal İletişim Bilgileri</Text>
        <View style={{ width: 40 }} />
      </View>
        ...
        <ScrollView contentContainerStyle={styles.container}>
          <View style={[styles.formCard, currentTheme === 'dark' && { backgroundColor: '#1e293b' }]}>
            <View style={styles.inputGroup}>
              <Text style={[styles.label, currentTheme === 'dark' && { color: '#94a3b8' }]}>Kurumsal Telefon</Text>
              <TextInput
          ...
          <Text style={[styles.infoText, currentTheme === 'dark' && { color: '#64748b' }]}>
            * Bu numara, kurumsal atık toplama süreçlerinde iletişim sağlamak ve hesap güvenliğiniz için kullanılacaktır.
          </Text>
        </ScrollView>
